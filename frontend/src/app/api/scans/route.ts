...
export async function GET() {
  const rows = await db
    ...
    .from(scans)
    .innerJoin(products, eq(products.id, scans.productId))
...
export async function POST(req: Request) {
  ...
  const { productId, image, backImage } = parsed.data;
  ...
  const { extracted, confidence, regions } = ocr;

  // 3. verify
  const result = await runVerification(productId, extracted, confidence, null, scan.id);
